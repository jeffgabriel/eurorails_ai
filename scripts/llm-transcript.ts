...
function formatTurn(entry: TurnEntry, showFullPrompts: boolean): string {
  ...
  if (entry.tripPlanning) {
    ...
    if (tp.candidates.length > 0 && tp.chosen >= 0 && tp.chosen < tp.candidates.length) {
      const c = tp.candidates[tp.chosen];
      lines.push(`   Chosen: Route ${tp.chosen + 1} of ${tp.candidates.length} — ${c.stops.join('→')} (score: ${c.score}, ${c.estimatedTurns}T, ${c.buildCostEstimate}M build)`);
    }
    if (tp.llmReasoning) {

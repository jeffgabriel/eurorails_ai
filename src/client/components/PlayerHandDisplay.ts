...
export class PlayerHandDisplay {
  ...
  public async update(isDrawingMode: boolean = false, currentTrackCost: number = 0, targetContainer: Phaser.GameObjects.Container): Promise<void> {
    ...
    const handHeight = this.isCollapsed ? this.STATUS_BAR_HEIGHT : layoutInfo.handHeight;
    
    // Create a container for the hand area that will hold all elements
    const handArea = this.scene.add.container(0, this.scene.scale.height - handHeight);
    ...
    if (this.isCollapsed) {
      ...
      const trainCardContainer = this.scene.add.container(layoutInfo.trainCardX, layoutInfo.trainCardY);
      handArea.add(trainCardContainer);
      (handArea as any).trainCardContainer = trainCardContainer;
      
      const playerInfoContainer = this.scene.add.container(layoutInfo.playerInfoX, layoutInfo.playerInfoY);
      ...
      this.createPlayerInfoSection(isDrawingMode, currentTrackCost, handArea, layoutInfo);
  ...
  private calculateLayout(): {
    ...
    const totalRequiredWidth = this.LEFT_BUFFER + this.CARDS_CONTAINER_MAX_WIDTH + this.CONTAINER_SPACING + trainCardWidth + this.CONTAINER_SPACING + playerInfoWidth + this.PADDING;
    ...
    const availableCardsWidth = containersSideBySide 
      ...
      : Math.max(this.CARDS_CONTAINER_MIN_WIDTH, Math.min(this.CARDS_CONTAINER_MAX_WIDTH, screenWidth - this.LEFT_BUFFER - this.PADDING));
    
    // Determine if cards should stack within their container
    const cardsRequiredWidth = (this.CARD_WIDTH * numCards) + (this.CARD_SPACING_HORIZONTAL * (numCards - 1));
    ...
    if (containersSideBySide) {
      ...
      const cardsHeight = cardsStacked 
        ? (this.CARD_HEIGHT * numCards) + (this.CARD_SPACING_VERTICAL * (numCards - 1)) + (this.PADDING * 2)
      ...
      const requiredInfoWidth = trainCardWidth + this.CONTAINER_SPACING + playerInfoWidth;
  ...
  private createHandBackground(targetContainer: Phaser.GameObjects.Container, height: number): void {
  ...
  private async createDemandCardSection(targetContainer: Phaser.GameObjects.Container, layoutInfo: ReturnType<typeof this.calculateLayout>): Promise<void> {
    ...
    const cardsContainer = this.scene.add.container(layoutInfo.cardsContainerX, layoutInfo.cardsContainerY);
    ...
    for (let i = 0; i < cardsToShow; i++) {
      ...
      if (layoutInfo.cardsStacked) {
        ...
        y = layoutInfo.cardStartY + (i * (this.CARD_HEIGHT + layoutInfo.cardSpacing));
      } else {
        // Cards in a row
        x = layoutInfo.cardStartX + (i * (this.CARD_WIDTH + layoutInfo.cardSpacing));
        y = layoutInfo.cardStartY;
      }
      
      const card = i < currentPlayer.hand.length ? currentPlayer.hand[i] : undefined;
  ...
  private createTrainSection(targetContainer: Phaser.GameObjects.Container, layoutInfo: ReturnType<typeof this.calculateLayout>): void {
    // Guard against race condition when gameStateService is null
    if (!this.gameStateService) {
    ...
    try {
      ...
      if (!trainCardContainer) {
        console.error('PlayerHandDisplay.createTrainSection: Train card container not found');
        return;
      }
  ...
  private createPlayerInfoSection(isDrawingMode: boolean, currentTrackCost: number, targetContainer: Phaser.GameObjects.Container, layoutInfo: ReturnType<typeof this.calculateLayout>): void {
    // Guard against race condition when gameStateService is null
    if (!this.gameStateService) {
    ...
    if (!playerInfoContainer) {
      console.error('PlayerHandDisplay.createPlayerInfoSection: Player info container not found');
      return;
    }
  ...
  private createToggleButton(targetContainer: Phaser.GameObjects.Container, handHeight: number): void {
    ...
    // Make it interactive
    const hitArea = this.scene.add.rectangle(buttonX, buttonY, 40, 40, 0x000000, 0);
  ...
  private createStatusBar(targetContainer: Phaser.GameObjects.Container): void {
    ...
    // Add mini train icon
    if (currentPlayer.trainType) {
      const trainType = currentPlayer.trainType.toLowerCase().replace(/[\s-]+/g, '');
      try {

...
describe('RouteEnrichmentAdvisor.enrich', () => {
  ...
  // ── JIRA-184: Enrichment validate call must not reorder stops ─────────────

  it('JIRA-184: calls RouteValidator.validate with reorderByProximity: false', async () => {
    // When the LLM inserts a stop, validate() should be called with the opt-out flag
    // so the enrichment advisor's intended order is preserved.
    const insertResponse: RouteEnrichmentSchema = {
      decision: 'insert',
      insertions: [{
        afterStopIndex: 0,
        ...
        loadType: 'Coal',
        city: 'Lyon',
        reasoning: 'Lyon is on the way',
      }],
      reasoning: 'Add Lyon stop',
    };
    const brain = makeMockBrain(JSON.stringify(insertResponse));
    const route = makeRoute();

    await RouteEnrichmentAdvisor.enrich(route, makeSnapshot(), makeContext(), brain, testGrid);

    // validate must have been called with reorderByProximity: false
    expect(RouteValidator.validate).toHaveBeenCalledWith(
      expect.objectContaining({ currentStopIndex: 0 }),
      expect.anything(),
      expect.anything(),
  ...
  it('JIRA-184: enriched route stop order is preserved after validate (Haiku turn-5 regression)', async () => {
    // Simulate the scenario: LLM inserts Holland and Wroclaw stops.
    // applyDecision produces: [P-Praha(0), P-Holland(1), D-Bruxelles(2), P-Wroclaw(3)]
    // With reorderByProximity: false, validate must NOT reorder these.
    const insertResponse: RouteEnrichmentSchema = {
      decision: 'insert',
      insertions: [
        {
          ...
          loadType: 'Cheese',
          city: 'Holland',
          reasoning: 'Pick up Cheese in Holland',
        },
        {
          afterStopIndex: 1,
          action: 'pickup',
          loadType: 'Steel',
          city: 'Wroclaw',
          reasoning: 'Pick up Steel in Wroclaw',
        },
      ],
      reasoning: 'Enrich with Holland and Wroclaw stops',
    };

    const extendedGrid: GridPoint[] = [
      ...testGrid,
      gp(4, 0, TerrainType.MajorCity, 'Praha'),
      gp(4, 1, TerrainType.MajorCity, 'Holland'),
      gp(5, 0, TerrainType.MajorCity, 'Bruxelles'),
      gp(5, 1, TerrainType.MajorCity, 'Wroclaw'),
    ];

    const brain = makeMockBrain(JSON.stringify(insertResponse));
    // Base route: [P-Praha, D-Bruxelles]
    const route = makeRoute([
      { action: 'pickup', loadType: 'Beer', city: 'Praha' },
      { action: 'deliver', loadType: 'Beer', city: 'Bruxelles' },
    ]);

    // After applyDecision: [P-Praha(0), P-Holland(1), D-Bruxelles(2), P-Wroclaw(3)]
    // validate returns valid with no pruning — order must match the LLM-intended order
    jest.mocked(RouteValidator.validate).mockReturnValue({ valid: true, errors: [] });

    const result = await RouteEnrichmentAdvisor.enrich(route, makeSnapshot(), makeContext(), brain, extendedGrid);

    // LLM-intended order must be preserved
    expect(result.stops[0]).toMatchObject({ action: 'pickup', city: 'Praha' });
    expect(result.stops[1]).toMatchObject({ action: 'pickup', city: 'Holland' });
    expect(result.stops[2]).toMatchObject({ action: 'deliver', city: 'Bruxelles' });
    expect(result.stops[3]).toMatchObject({ action: 'pickup', city: 'Wroclaw' });
  });
});

...
describe('RouteValidator', () => {
  ...
  // ── JIRA-184: reorderByProximity opt-out ────────────────────────────────────

  describe('JIRA-184: reorderByProximity option', () => {
    beforeEach(() => {
      // Set up grid points so reorder has coordinates to work with
      mockGridPoints.set('28,54', { row: 28, col: 54, terrain: TerrainType.MajorCity, name: 'Szczecin' });
      mockGridPoints.set('40,25', { row: 40, col: 25, terrain: TerrainType.MajorCity, name: 'Praha' });
      mockGridPoints.set('20,15', { row: 20, col: 15, terrain: TerrainType.MajorCity, name: 'Holland' });
      mockGridPoints.set('15,10', { row: 15, col: 10, terrain: TerrainType.MajorCity, name: 'Bruxelles' });
      mockGridPoints.set('35,60', { row: 35, col: 60, terrain: TerrainType.MajorCity, name: 'Wroclaw' });
    });

    it('default behavior (no options) still reorders stops by proximity', () => {
      // Bot at Szczecin (28,54). Wroclaw (35,60) is closer than Praha (40,25).
      // Default reorder should move Wroclaw before Praha.
      mockEstimateHopDistance.mockImplementation(
        (fromRow: number, _fromCol: number, toRow: number, _toCol: number) => {
          if (fromRow === 28 && toRow === 40) return 20; // Szczecin→Praha: far
          if (fromRow === 28 && toRow === 35) return 5;  // Szczecin→Wroclaw: close
          if (fromRow === 28 && toRow === 20) return 15; // Szczecin→Holland: medium
          if (fromRow === 28 && toRow === 15) return 18; // Szczecin→Bruxelles: far
          return 10;
        },
      );
      ...
      const route = makeRoute({
        stops: [
          { action: 'pickup', loadType: 'Beer', city: 'Praha' },     // index 0 — far
          { action: 'pickup', loadType: 'Steel', city: 'Wroclaw' },  // index 1 — close
          { action: 'deliver', loadType: 'Beer', city: 'Bruxelles', demandCardId: 1, payment: 20 },
          { action: 'deliver', loadType: 'Steel', city: 'Holland', demandCardId: 2, payment: 15 },
        ],
      });
      const context = makeContext({ demands: [coalDemand, steelDemand] });
      const snapshot = makeSnapshot();
      snapshot.bot.position = { row: 28, col: 54 }; // at Szczecin

      // Default: reorder fires → Wroclaw pickup should precede Praha pickup
      const result = RouteValidator.validate(route, context, snapshot);
      expect(result.valid).toBe(true);
      const finalStops = result.prunedRoute?.stops ?? route.stops;
      const wroclawIdx = finalStops.findIndex(s => s.city === 'Wroclaw');
      const prahaIdx = finalStops.findIndex(s => s.city === 'Praha');
      expect(wroclawIdx).toBeLessThan(prahaIdx);
    });

    it('reorderByProximity: false preserves LLM-intended stop order', () => {
      // Haiku turn-5 regression: bot at Szczecin, route [P-Praha, P-Holland, D-Bruxelles, P-Wroclaw]
      // Wroclaw is geographically closer → reorder would put it first → invariant fires.
      // With opt-out, the order must be preserved exactly.
      mockEstimateHopDistance.mockImplementation(
        (fromRow: number, _fromCol: number, toRow: number, _toCol: number) => {
          if (fromRow === 28 && toRow === 40) return 20; // Szczecin→Praha: far
          if (fromRow === 28 && toRow === 20) return 15; // Szczecin→Holland: medium
          if (fromRow === 28 && toRow === 15) return 18; // Szczecin→Bruxelles: far
          if (fromRow === 28 && toRow === 35) return 5;  // Szczecin→Wroclaw: close (would normally come first)
          return 10;
        },
      );

      const beerDemand = makeDemand({
        cardIndex: 1,
        loadType: 'Beer',
        supplyCity: 'Praha',
        deliveryCity: 'Bruxelles',
      ...
      const cheeseDemand = makeDemand({
        cardIndex: 2,
        loadType: 'Cheese',
        supplyCity: 'Holland',
        deliveryCity: 'Wroclaw',
        payout: 15,
        isSupplyOnNetwork: true,
        isDeliveryOnNetwork: true,
        estimatedTrackCostToSupply: 0,
      ...
      // LLM-intended order: Praha → Holland → Bruxelles → Wroclaw
      const route = makeRoute({
        stops: [
          { action: 'pickup', loadType: 'Beer', city: 'Praha' },          // 0
          { action: 'pickup', loadType: 'Cheese', city: 'Holland' },       // 1
          { action: 'deliver', loadType: 'Beer', city: 'Bruxelles', demandCardId: 1, payment: 20 }, // 2
          { action: 'deliver', loadType: 'Cheese', city: 'Wroclaw', demandCardId: 2, payment: 15 }, // 3
        ],
      });
      const context = makeContext({ demands: [beerDemand, cheeseDemand] });
      const snapshot = makeSnapshot();
      snapshot.bot.position = { row: 28, col: 54 }; // at Szczecin

      const result = RouteValidator.validate(route, context, snapshot, { reorderByProximity: false });

      expect(result.valid).toBe(true);
      // Stop order must be exactly preserved
      const finalStops = result.prunedRoute?.stops ?? route.stops;
      expect(finalStops[0]).toMatchObject({ action: 'pickup', city: 'Praha' });
      expect(finalStops[1]).toMatchObject({ action: 'pickup', city: 'Holland' });
      expect(finalStops[2]).toMatchObject({ action: 'deliver', city: 'Bruxelles' });
      expect(finalStops[3]).toMatchObject({ action: 'deliver', city: 'Wroclaw' });
    });
  });
});

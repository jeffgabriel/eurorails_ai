...
/**
 * Thrown by PlayerService when an action is blocked by an active event-card
 * restriction. The closed `code` union enables type-safe dispatch in TurnExecutor.
 *
 * JIRA-256 Phase 4.
 */
export type ActionRestrictionErrorCode =
  ...
  | 'SNOW_HALF_RATE_EXCEEDED'  // half-rate enforced bot-side only
  | 'BUILD_BLOCKED'
  | 'LOST_TURN'
...
export class PlayerService {
  ...
  static async deliverLoadForUser(
    ...
    try {
      ...
      if (deliveryRestrictions.length > 0) {
        const cityKey = getCityMilepointKey(city);
        const deliveryVerdict = isPickupDeliveryBlocked(deliveryRestrictions, cityKey);
        if (deliveryVerdict.blocked) {
          console.warn(
  ...
  static async moveTrainForUser(args: {
    ...
    try {
      ...
      {
        // blocked_terrain: Snow — zone is pre-filtered to blocked terrain mileposts only.
        const movVerdict = isMovementBlockedAtDest(movementRestrictions, destKey);
  ...
  static async pickupLoadForPlayer(
    ...
    try {
      ...
      if (pickupRestrictions.length > 0 && cityName) {
        ...
        const pickupVerdict = isPickupDeliveryBlocked(pickupRestrictions, cityKey);
        if (pickupVerdict.blocked) {
          console.warn(
  ...
  static async buildTrackForPlayer(
    ...
    try {
      ...
      for (const seg of newSegments) {
        ...
        const buildVerdict = isBuildBlockedAtMilepost(buildRestrictions, segDestKey, playerId);
        if (buildVerdict.blocked) {
          if (buildVerdict.reason === 'no_build_for_player') {
            console.warn(
            ...
            console.warn(
              `[PlayerService] Build rejected by event restriction: type=blocked_terrain gameId=${gameId} playerId=${playerId} destKey=${segDestKey}`,
            );
            throw new ActionRestrictionError(
              'BUILD_BLOCKED',
      ...
      for (const seg of newSegments) {
        ...
        if (floodVerdict.blocked) {
          console.warn(
            `[PlayerService] Build rejected by event restriction: type=flood_rebuild gameId=${gameId} playerId=${playerId} river=${floodVerdict.river}`,
          );
          throw new ActionRestrictionError(
            'FLOOD_BRIDGE_REBUILD_BLOCKED',
            `Build blocked: cannot rebuild track across the ${floodVerdict.river} river while Flood event is active`,
          );
        }
      }
      // ── End restriction checks ────────────────────────────────────────────
